...
mkdir -p "$temp_dir/htdocs/custom/$module_name"
mkdir -p "$temp_dir/htdocs/custom/$module_name/core/modules"
mkdir -p "$temp_dir/htdocs/custom/$module_name/img"
mkdir -p "$temp_dir/htdocs/custom/$module_name/js"
mkdir -p "$temp_dir/htdocs/custom/$module_name/css"
mkdir -p "$temp_dir/htdocs/custom/$module_name/lang"
mkdir -p "$temp_dir/htdocs/custom/$module_name/ajax"
mkdir -p "$temp_dir/htdocs/custom/$module_name/class"

# Copia os arquivos para a estrutura correta
cp -r "$module_dir/"* "$temp_dir/htdocs/custom/$module_name/" 2>/dev/null

# Cria o arquivo .norescan
touch "$temp_dir/htdocs/custom/$module_name/.norescan"

# Cria o arquivo .newmodule
touch "$temp_dir/htdocs/custom/$module_name/.newmodule"
